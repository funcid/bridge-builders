...
enum class WeekRewards(val reward: DailyReward, val give: (User) -> Any) {
    ONE(DailyReward("§e250 Эфира", item { type = org.bukkit.Material.GOLD_INGOT }), { it.giveEther(250) }),
    TWO(DailyReward("§aПрефикс Мостовой", NameTagType.BRIDGE.getIcon()), { it.stat.nameTags.add(NameTagType.BRIDGE.name) }),
    THREE(
    ...
    SIX(DailyReward("§e5`000 Эфира", item {
        type = Material.GOLD_INGOT
        enchant(org.bukkit.enchantments.Enchantment.DAMAGE_ALL, 1)
    }), { it.giveEther(5000) }),
    SEVEN(
        DailyReward("§6Легендарный лутбокс §f+ §e5`000 Эфира", item {
            type = Material.CLAY_BALL
            enchant(org.bukkit.enchantments.Enchantment.DAMAGE_ALL, 1)
            nbt("other", "enderchest1")
        }),
